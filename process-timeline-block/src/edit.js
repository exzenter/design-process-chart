...
import {
	PanelBody, PanelRow, TextControl, SelectControl, RangeControl,
	ToggleControl, Button, ButtonGroup, ToolbarGroup, ToolbarButton,
	ColorPicker, ColorIndicator,
	BaseControl, TextareaControl, Dropdown,
} from '@wordpress/components';
import './editor.scss';
import { renderTimeline } from './timeline-renderer';
import { DEFAULT_STEPS, DEFAULT_PHASES, DEFAULT_SETTINGS } from './default-data';

export default function Edit( { attributes, setAttributes } ) {
	const {
		timelineSteps, phases, settings, versions, activeVersion,
		responsiveBreakpoint, mobileLayout, desktopLayout,
		showViewToggle, showVersionButtons,
	} = attributes;
	...
	useEffect( () => {
		if ( containerRef.current && timelineSteps && timelineSteps.length > 0 && settings && phases ) {
	...
	const deleteBubble = () => {
		...
		setAttributes( { timelineSteps: ( timelineSteps || [] ).filter( ( s ) => s.id !== selectedBubble ) } );
		setSelectedBubble( '' );
	};
		...
		<div { ...blockProps }>
			<InspectorControls>
				...
				<PanelBody title="Bubble Colors" initialOpen={ false }>
					{ phaseKeys.map( ( key ) => (
						<BaseControl key={ key } label={ ( phases || DEFAULT_PHASES )[ key ]?.name || key }>
							<PanelRow>
								<ColorIndicator colorValue={ settings?.colors?.[ key ] || '#ccc' } />
								<Dropdown
									renderToggle={ ( { isOpen, onToggle } ) => (
										<Button onClick={ onToggle } aria-expanded={ isOpen } variant="secondary" size="small">
											Change
										</Button>
									) }
									renderContent={ () => (
				...
				<PanelBody title="Timeline Line" initialOpen={ false }>
					<BaseControl label="Line Color">
						<Dropdown
							renderToggle={ ( { isOpen, onToggle } ) => (
								<PanelRow>
									<ColorIndicator colorValue={ settings?.timelineColor || '#333' } />
									<Button onClick={ onToggle } aria-expanded={ isOpen } variant="secondary" size="small">Change</Button>
								</PanelRow>
							) }
							renderContent={ () => (
								<ColorPicker color={ settings?.timelineColor || '#333' } onChange={ ( c ) => updateSetting( 'timelineColor', c ) } />
							) }
						/>
					</BaseControl>
					<RangeControl
				...
				<PanelBody title="Connection Lines" initialOpen={ false }>
					<BaseControl label="Color">
						<Dropdown
							renderToggle={ ( { isOpen, onToggle } ) => (
								<PanelRow>
									<ColorIndicator colorValue={ settings?.connectionColor || '#999' } />
									<Button onClick={ onToggle } aria-expanded={ isOpen } variant="secondary" size="small">Change</Button>
								</PanelRow>
							) }
							renderContent={ () => (
								<ColorPicker color={ settings?.connectionColor || '#999' } onChange={ ( c ) => updateSetting( 'connectionColor', c ) } />
							) }
						/>
					</BaseControl>
					<RangeControl
				...
				<PanelBody title="Text & Indicators" initialOpen={ false }>
					<BaseControl label="Text Color">
						<Dropdown
							...
							renderContent={ () => (
								<ColorPicker color={ settings?.textColor || '#333' } onChange={ ( c ) => updateSetting( 'textColor', c ) } />
							) }
						/>
					</BaseControl>
					<SelectControl
					...
					<BaseControl label="Indicator Color">
						<Dropdown
							renderToggle={ ( { isOpen, onToggle } ) => (
								<PanelRow>
									<ColorIndicator colorValue={ settings?.indicatorColor || '#666' } />
									<Button onClick={ onToggle } aria-expanded={ isOpen } variant="secondary" size="small">Change</Button>
								</PanelRow>
							) }
							renderContent={ () => (
								<ColorPicker color={ settings?.indicatorColor || '#666' } onChange={ ( c ) => updateSetting( 'indicatorColor', c ) } />
							) }
						/>
					</BaseControl>
					<RangeControl
				...
				<PanelBody title="Hover Effects" initialOpen={ false }>
					...
					<BaseControl label="Hover Line Color">
						<Dropdown
							renderToggle={ ( { isOpen, onToggle } ) => (
								<PanelRow>
									<ColorIndicator colorValue={ settings?.connectionHoverColor || '#e63946' } />
									<Button onClick={ onToggle } aria-expanded={ isOpen } variant="secondary" size="small">Change</Button>
								</PanelRow>
							) }
							renderContent={ () => (
								<ColorPicker color={ settings?.connectionHoverColor || '#e63946' } onChange={ ( c ) => updateSetting( 'connectionHoverColor', c ) } />
							) }
						/>
					</BaseControl>
					<RangeControl
				...
				<PanelBody title="Bubble Editor" initialOpen={ false }>
					<SelectControl
						...
						options={ [
							...
							...( timelineSteps || [] ).map( ( s ) => ( {
								label: `${ s.id } - ${ ( phases || {} )[ s.phase ]?.name || s.phase } (x:${ s.x })`,
								value: s.id,
							} ) ),
						] }
					...
					{ currentStep && (
						<div className="ppt-bubble-editor">
							...
							<div style={ { marginTop: '12px', display: 'flex', gap: '8px' } }>
								<Button variant="primary" onClick={ () => {} } style={ { flex: 1 } } disabled>
									Auto-saved
								</Button>
								<Button variant="secondary" isDestructive onClick={ deleteBubble } style={ { flex: 1 } }>
									Delete
								</Button>
							</div>
						</div>
					...
					<Button variant="secondary" onClick={ addBubble } style={ { width: '100%', marginTop: '12px' } }>
						+ Add New Bubble
					</Button>
				</PanelBody>
...
function BubbleTaskEditor( { label, tasks, stepId, stepX, side, updateStep } ) {
	...
	const addTask = () => {
		const defaultLineY = side === 'preface' ? -7 - taskArray.length * 1.5 : 7 + taskArray.length * 1.5;
		const newTask = {
			label: 'New Task',
			fontSize: 'M',
			fontWeight: 'regular',
	...
	return (
		<div className="ppt-task-section">
			<div className="ppt-task-section-header">
				<strong>{ label } Descriptions ({ taskArray.length })</strong>
				{ taskArray.length < 3 && (
					<Button variant="link" onClick={ addTask } size="small">+ Add</Button>
				) }
			</div>
			{ taskArray.map( ( task, idx ) => (
				<div key={ idx } className="ppt-task-item">
					<div className="ppt-task-item-header">
						...
						<Button variant="link" isDestructive onClick={ () => removeTask( idx ) } size="small">Remove</Button>
					</div>
					<TextareaControl
						label="Label"
						value={ task.label || '' }
...
function VersionManager( { versions, activeVersion, onSave, onLoad, onDelete, onImport, onExport } ) {
		...
		<div className="ppt-version-manager">
			<div style={ { display: 'flex', gap: '8px', marginBottom: '12px' } }>
				<TextControl
					...
					style={ { flex: 1 } }
				/>
				<Button variant="primary" onClick={ handleSave } size="small">Save</Button>
			</div>

			{ versionKeys.length > 0 && (
				<div className="ppt-version-list">
					{ versionKeys.map( ( name ) => (
						<div key={ name } className={ `ppt-version-item ${ name === activeVersion ? 'active' : '' }` }>
							<Button
			...
			<div style={ { display: 'flex', gap: '8px', marginTop: '12px' } }>
				<Button variant="secondary" onClick={ onImport } size="small" style={ { flex: 1 } }>Import</Button>
				<Button variant="secondary" onClick={ onExport } size="small" style={ { flex: 1 } }>Export</Button>
			</div>
		</div>
	);
}
